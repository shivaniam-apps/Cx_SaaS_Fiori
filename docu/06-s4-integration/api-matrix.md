...
| `E070` / `E07T` / `E071` table read on the **follow-on** system (S10 `ZADO_C_TRANSPORT_STATUS`) | table read, not yet run on RD1 | Import evidence without TMS: a request exists in the target's `E070` only once tp imported it (object list travels with the import), status `R`. tp return codes stay in the TMS logs (idea I30). |
