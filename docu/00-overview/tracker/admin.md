...
### 2026-09-18
- A10 written as three runbooks, each verified against the code it describes (mta.yaml, env.js knobs and defaults, task-runner states, checkTargetSystemConnection stages, ZCL_ADO_ACT_HTTP publication note, ZADO_CFG_INIT). Findings while writing: the tenant route for a subscribed subdomain must be mapped by hand (`cf map-route`) because the subscription callback only returns the URL (idea I30, belongs to T2); no confirmed authorization-object list exists for the S/4 technical user (left to T6 with an ST01-trace instruction); SystemInfo cannot yet confirm the pseudonym secret (I29 stays open). A5-A9 Accomplished lines now carry their PR numbers (#25, #27, #31, #32, #35).
