...
- [x] O8 Adoption Cockpit KPI strip (promoted from I1): queryDashboardSummary (one read, grouped counts per journey stage at the database, proposals scoped to the current analysis run per system), per-stage KPI tiles on the Dashboard with a target-system scope and a next-step prompt, click-through into Proposals / Activation Runs / Transports with the same status bucket pre-applied (new URL filters on those pages, bucket status params on queryProposals / queryActivationRuns / queryTransportRequests) — this PR, 2026-09-18
