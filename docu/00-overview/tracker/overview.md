...
- [~] O10 Activation Runs KPI cards as status filters (promoted I7): queryActivationRuns gains a status parameter so card and slice use the same server expression; in-content scope gesture applies and writes the draft — branch feat/runs-kpi-filters (stacked on feat/adops-toast, PR #46), started 2026-09-18
