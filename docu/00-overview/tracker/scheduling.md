...
- [x] S1 Connection check covers the activation service: per-endpoint verdicts (USAGE read service, ACTIVATE write unit) in checkTargetSystemConnection / getBackendCapabilities, activation judged against the environment (reachable on DEV, UNPUBLISHED required on QA/PROD, EXPOSED = safety finding), persisted in TargetSystems.lastCheckEndpointsJson, badges on the Target Systems page — this PR, 2026-09-17
